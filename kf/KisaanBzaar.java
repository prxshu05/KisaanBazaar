...
class Notification {
    ...
    private String timestamp; // Changed to String
...
class Negotiation {
    ...
    public Negotiation(String orderId, String buyerId, String farmerId, double proposedPrice) {
        this.id = UUID.randomUUID().toString();
...
class OrderDTO {
    ...
    public OrderDTO(String id, String buyerId, String productId, String productName, int quantity, double totalPrice, String status, String createdAt) {
        this.id = id;
        this.buyerId = buyerId;
        this.productId = productId;
    ...
    public Object[] toRow() {
        return new Object[]{id, buyerId, productId, productName, quantity, totalPrice, status, createdAt};
    }
}
...
class NegotiationDTO {
    ...
    public NegotiationDTO(String id, String orderId, String buyerId, String farmerId, double proposedPrice, String status) {
        this.id = id;
        this.orderId = orderId;
        this.buyerId = buyerId;
        this.farmerId = farmerId;
    ...
    public Object[] toRow() {
        return new Object[]{id, orderId, buyerId, farmerId, proposedPrice, status};
    }
}
...
// Database Connection (Enhanced with Logging)
class DBConnection {
...
class UserDAO {
    ...
    public void ensureAdminAccount() throws SQLException {
        ...
        if (!doesEmailExist(ADMIN_EMAIL)) {
            User admin = new User("Admin", ADMIN_EMAIL, ADMIN_PASSWORD, "Admin");
            saveUser(admin);
        ...
        try {
            ...
            if (rs.next()) {
                ...
                if (password.equals(storedPassword)) {
                    ...
                    User user = new User(rs.getString("name"), rs.getString("email"), storedPassword, rs.getString("role"));
    ...
    public void resetPassword(String email) throws SQLException {
        ...
        try {
            conn = DBConnection.getConnection();
            stmt = conn.prepareStatement(sql);
            stmt.setString(1, "reset123"); // Default password after reset
...
class OrderDAO {
    ...
    public static final String[] ORDER_COLUMNS = {"ID", "Buyer ID", "Product ID", "Product Name", "Quantity", "Total Price", "Status", "Created At"};
    ...
    public List<OrderDTO> getOrdersByUser(String userId, String role) throws SQLException {
        ...
        if (role.equals("Farmer")) {
            sql = "SELECT o.id, o.buyer_id, o.product_id, p.name AS product_name, o.quantity, o.total_price, o.status, o.created_at " +
                  "FROM Orders o JOIN Products p ON o.product_id = p.id WHERE p.farmer_id = ?";
        } else if (role.equals("Buyer")) {
            sql = "SELECT o.id, o.buyer_id, o.product_id, p.name AS product_name, o.quantity, o.total_price, o.status, o.created_at " +
                  "FROM Orders o JOIN Products p ON o.product_id = p.id WHERE o.buyer_id = ?";
        } else {
            sql = "SELECT o.id, o.buyer_id, o.product_id, p.name AS product_name, o.quantity, o.total_price, o.status, o.created_at " +
        ...
        try {
            ...
            while (rs.next()) {
                OrderDTO order = new OrderDTO(
                    ...
                    rs.getString("status"),
                    rs.getTimestamp("created_at").toString() // Convert Timestamp to String
                );
                orders.add(order);
            }
    ...
    public List<OrderDTO> getOrdersWithFilters(String status, String startDate, String endDate) throws SQLException {
        List<OrderDTO> orders = new ArrayList<>();
        String sql = "SELECT o.id, o.buyer_id, o.product_id, p.name AS product_name, o.quantity, o.total_price, o.status, o.created_at " +
                     "FROM Orders o JOIN Products p ON o.product_id = p.id WHERE 1=1";
        if (!status.isEmpty()) sql += " AND o.status = ?";
        if (!startDate.isEmpty() && !endDate.isEmpty()) sql += " AND o.created_at BETWEEN ? AND ?";
        LOGGER.info("Executing query to get orders with filters: " + sql + " with status: " + status + ", startDate: " + startDate + ", endDate: " + endDate);
        Connection conn = null;
        PreparedStatement stmt = null;
        ResultSet rs = null;
        try {
...
class NegotiationDAO {
    ...
    public static final String[] NEGOTIATION_COLUMNS = {"ID", "Order ID", "Buyer ID", "Farmer ID", "Proposed Price", "Status"};

    public void saveNegotiation(Negotiation negotiation) throws SQLException {
        String sql = "INSERT INTO Negotiations (id, order_id, buyer_id, farmer_id, proposed_price, status) VALUES (?, ?, ?, ?, ?, ?)";
        ...
        try {
            ...
            stmt.setString(6, negotiation.getStatus());
            int rowsAffected = stmt.executeUpdate();
...
class NotificationService {
    ...
    public void notifyPriceProposal(String farmerId, String orderId, double proposedPrice) throws SQLException {
        notifyUser(farmerId, "Buyer proposed $" + proposedPrice + " for order #" + orderId + ". Accept or reject?");
    }
...
class OrderService {
    ...
    public void placeOrder(Order order, boolean isWholesale) throws SQLException {
        ...
        if (order.getQuantity() <= availableQty) {
            order.setTotalPrice(order.getQuantity() * price * 1.05);
            OrderDAO orderDAO = new OrderDAO();
            orderDAO.saveOrder(order);
            productDAO.updateQuantity(order.getProductId(), availableQty - order.getQuantity());
            RevenueDAO revenueDAO = new RevenueDAO();
            revenueDAO.saveRevenue(order.getId(), order.getTotalPrice() * 0.05);
            NotificationService notificationService = new NotificationService();
            notificationService.notifyOrderPlaced(product.getFarmerId(), order.getBuyerId(), product.getName());
...
class NegotiationService {
    ...
    public void proposePrice(String orderId, String buyerId, String farmerId, double proposedPrice) throws SQLException {
        LOGGER.info("Proposing price for order: " + orderId + ", buyerId: " + buyerId + ", farmerId: " + farmerId + ", proposedPrice: " + proposedPrice);
        Negotiation negotiation = new Negotiation(orderId, buyerId, farmerId, proposedPrice);
        NegotiationDAO dao = new NegotiationDAO();
        dao.saveNegotiation(negotiation);
        NotificationService notificationService = new NotificationService();
        notificationService.notifyPriceProposal(farmerId, orderId, proposedPrice);
        LOGGER.info("Price proposed for order: " + orderId);
    }

    public void attendToProposal(String negotiationId, String response, String buyerId, String orderId) throws SQLException {
...
class NotificationFrame extends JFrame {
    ...
    private void updateNotifications() {
        try {
            ...
            String[] display = notifications.stream()
                    .map(n -> (n.isRead() ? "[Read] " : "[Unread] ") + n.getMessage())
                    .toArray(String[]::new);
            notificationList.setListData(display);
        } catch (SQLException ex) {
            ex.printStackTrace();
    ...
    private void markAsRead() {
        ...
        if (selectedIndex >= 0) {
            try {
                NotificationDAO dao = new NotificationDAO();
                List<Notification> notifications = dao.getNotificationsByUser(userId);
                String notificationId = notifications.get(selectedIndex).getId();
...
class LoginFrame extends JFrame {
    ...
    public LoginFrame() {
        ...
        // Header panel with title and admin login button
        ...
        adminLoginButton.addActionListener(e -> {
            JOptionPane.showMessageDialog(this, 
                "Admin Credentials:\nEmail: " + ADMIN_EMAIL + "\nPassword: " + ADMIN_PASSWORD + "\n\nPlease enter these credentials to log in as Admin.",
                "Admin Login", 
...
class RegisterFrame extends JFrame {
    ...
    public RegisterFrame() {
        ...
        roleCombo = new JComboBox<>(new String[]{"Farmer", "Buyer"}); // Removed "Admin" option
        roleCombo.setFont(fieldFont);
        roleCombo.setBackground(Color.WHITE);
        roleCombo.setBorder(BorderFactory.createLineBorder(new Color(34, 139, 34), 1));
        gbc.gridx = 1;
    ...
    private void register() {
        ...
        try {
            ...
            User user = new User("User", email, password, role);
...
class FarmerFrame extends JFrame {
    ...
    private JPanel createSidebar() {
        ...
        String[] menuItems = {"Dashboard", "Add Product", "View Products", "Order Status", "Order History", "Negotiations", "Notifications"};
        for (String item : menuItems) {
            ...
            button.addActionListener(e -> {
                if (item.equals("Notifications")) {
                    ...
                    cardLayout.show(mainContentPanel, item.replace(" ", ""));
                }
            });
    ...
    private JPanel createDashboardPanel() {
        ...
        try {
            ...
            JPanel notificationCard = createSummaryCard("Unread Notifications", notificationDAO.getUnreadNotificationCount(farmerId));
        ...
        addProductButton.addActionListener(e -> cardLayout.show(mainContentPanel, "AddProduct"));
        quickActionsPanel.add(addProductButton);

        JButton viewOrdersButton = createStyledButton("View Orders");
        viewOrdersButton.addActionListener(e -> cardLayout.show(mainContentPanel, "OrderStatus"));
    ...
    private JPanel createAddProductPanel() {
        ...
        viewProductsButton.addActionListener(e -> cardLayout.show(mainContentPanel, "ViewProducts"));
    ...
    private JPanel createViewProductsPanel() {
        ...
        try {
            ...
            productTable.setModel(new ListTableModel(products, ProductDAO.PRODUCT_COLUMNS));
    ...
    private JPanel createOrderStatusPanel() {
        ...
        try {
            ...
            orderTable.setModel(new ListTableModel(orders, OrderDAO.ORDER_COLUMNS));
    ...
    private JPanel createOrderHistoryPanel() {
        ...
        try {
            OrderDAO orderDAO = new OrderDAO();
            List<OrderDTO> orders = orderDAO.getOrdersByUser(farmerId, "Farmer");
            historyTable.setModel(new ListTableModel(orders, OrderDAO.ORDER_COLUMNS));
    ...
    private JPanel createNegotiationsPanel() {
        ...
        try {
            ...
            negotiationTable.setModel(new ListTableModel(negotiations, NegotiationDAO.NEGOTIATION_COLUMNS));
    ...
    private void handleNegotiationResponse(JTable table, String response) {
        ...
        if (row >= 0) {
            ...
            try {
                ...
                negotiationService.attendToProposal(negotiationId, response, buyerId, orderId);
                NegotiationDAO negotiationDAO = new NegotiationDAO();
                List<NegotiationDTO> negotiations = negotiationDAO.getNegotiationsByFarmer(farmerId);
                table.setModel(new ListTableModel(negotiations, NegotiationDAO.NEGOTIATION_COLUMNS));
                JOptionPane.showMessageDialog(this, "Negotiation " + response + "!", "Success", JOptionPane.INFORMATION_MESSAGE);
            } catch (SQLException ex) {
                ex.printStackTrace();
...
class BuyerFrame extends JFrame {
    ...
    private JPanel createDashboardPanel() {
        ...
        browseProductsButton.addActionListener(e -> cardLayout.show(mainContentPanel, "BrowseProducts"));
        ...
        if (row >= 0) {
            ...
            if (quantityStr != null && ValidationUtils.isPositiveNumber(quantityStr)) {
                ...
                if (quantity > 0 && quantity <= availableQty) {
                    try {
                        Order order = new Order(buyerId, productId, quantity, 0); // Total price will be set in OrderService
                        OrderService orderService = new OrderService();
                        orderService.placeOrder(order, isWholesale);
                        JOptionPane.showMessageDialog(this, "Order placed successfully!", "Success", JOptionPane.INFORMATION_MESSAGE);
                        ProductDAO productDAO = new ProductDAO();
                        List<ProductDTO> products = productDAO.getProducts("", 0, 1000, "retail_price ASC");
                        table.setModel(new ListTableModel(products, ProductDAO.PRODUCT_COLUMNS));
                    } catch (SQLException ex) {
                        ex.printStackTrace();
    ...
    private void handleProposePrice(JTable table) {
        ...
        if (row >= 0) {
            ...
            String proposedPriceStr = JOptionPane.showInputDialog(this, "Enter your proposed price (Current: $" + currentPrice + "):", "Propose Price", JOptionPane.PLAIN_MESSAGE);
            if (proposedPriceStr != null && ValidationUtils.isPositiveNumber(proposedPriceStr)) {
                double proposedPrice = Double.parseDouble(proposedPriceStr);
                if (proposedPrice > 0) {
                    try {
                        Order order = new Order(buyerId, productId, 1, proposedPrice);
                        OrderDAO orderDAO = new OrderDAO();
                        orderDAO.saveOrder(order);
                        NegotiationService negotiationService = new NegotiationService();
                        negotiationService.proposePrice(order.getId(), buyerId, farmerId, proposedPrice);
...
class AdminFrame extends JFrame {
    ...
    private JPanel createSidebar() {
        ...
        String[] menuItems = {"Dashboard", "Order Management"};
        for (String item : menuItems) {
            JButton button = new JButton(item);
            button.setFont(SIDEBAR_FONT);
            button.setForeground(Color.WHITE);
            ...
            button.setAlignmentX(Component.LEFT_ALIGNMENT);
            button.setMaximumSize(new Dimension(Integer.MAX_VALUE, 40));
            button.addActionListener(e -> cardLayout.show(mainContentPanel, item.replace(" ", "")));
            button.addMouseListener(new java.awt.event.MouseAdapter() {
                public void mouseEntered(java.awt.event.MouseEvent evt) {
                    button.setBackground(new Color(50, 205, 50));
                }
    ...
    private JPanel createDashboardPanel() {
        ...
        manageOrdersButton.addActionListener(e -> cardLayout.show(mainContentPanel, "OrderManagement"));
...
class ListTableModel extends AbstractTableModel {
    ...
    public ListTableModel(List<Object[]> rows, String[] columnNames) {
        this.rows = rows;
        this.columnNames = columnNames;
    }

    // Constructor for List<ProductDTO>
    public ListTableModel(List<ProductDTO> products, String[] columnNames) {
        this.columnNames = columnNames;
    ...
    // Constructor for List<OrderDTO>
    public ListTableModel(List<OrderDTO> orders, String[] columnNames) {
        this.columnNames = columnNames;
        this.rows = new ArrayList<>();
    ...
    // Constructor for List<NegotiationDTO>
    public ListTableModel(List<NegotiationDTO> negotiations, String[] columnNames) {
        this.columnNames = columnNames;
        this.rows = new ArrayList<>();
